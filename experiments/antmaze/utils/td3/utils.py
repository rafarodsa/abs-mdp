...
def load(td3_agent, filename, map_location=None):
    map_location = torch.device(td3_agent.device) if map_location is None else map_location
    td3_agent.critic.load_state_dict(torch.load(filename + "_critic", map_location=map_location))
    td3_agent.critic_optimizer.load_state_dict(torch.load(filename + "_critic_optimizer", map_location=map_location))
    td3_agent.critic_target = copy.deepcopy(td3_agent.critic)

    td3_agent.actor.load_state_dict(torch.load(filename + "_actor", map_location=map_location))
    td3_agent.actor_optimizer.load_state_dict(torch.load(filename + "_actor_optimizer", map_location=map_location))
