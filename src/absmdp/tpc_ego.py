...
class RSSMAbstraction(pl.LightningModule):
    ...
    def __init__(self, cfg: TrainerConfig):
        ...
        self.target = build_model(cfg.model.encoder.features)
    ...
    def _run_step(self, s, a, next_s, reward, duration, masks, success):
        if not self.initialized and isinstance(s, dict):
            with torch.no_grad():
                ...
                self.target.mlp_keys = self.encoder.mlp_keys
                self.target.cnn_keys = self.encoder.cnn_keys
                self.target.initialized = True
        ...
        grounding_loss = self.grounding_loss_normal(z[_mask], z[_mask])
    ...
    def step(self, batch, batch_idx):
        s, a, reward, next_s, duration, success, done, masks = batch
...
def run(cfg, ckpt=None, args=None):
    ...
    model = RSSMAbstraction(cfg.world_model)
    print(model)
    data = TrajaectoryReplayBufferStored(int(cfg.world_model.data.buffer_size), save_path=f'{args.data_path}/data', device='gpu')
