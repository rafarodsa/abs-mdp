...
class MarkovAMDP(gym.Env, nn.Module):
    ...
    def step(self, action):
        ...
        # done = done or initset.sum(-1, keepdim=True) == 0
    ...
    @torch.no_grad()
    def validate(self, neps):
        # sample data.validation
        eps = self.data.sample_validation(neps)
        ...
        metrics = list(logs.items())
        metrics = [f'val_{k}: {v:.3f}' for k, v in metrics if k != 'step']
        metrics.append(f'val_ep_likelihood: {loglikelihood:.3f}')
        metrics = ' | '.join(metrics)
        print(f'[validation losses] {metrics}')
    
        logs = {f'val/{k}':v for k,v in logs.items()}
        logs['val/ep_likelihood'] = loglikelihood
    ...
    def termination_loss(self, done, next_z):
        ...
        n_pos = target.sum()
        n_neg = (1-target).sum()
        tp = (done_pred * target).sum()
        tn = ((1-done_pred) * (1-target)).sum()
        fp = (done_pred * (1-target)).sum()
        fn = ((1-done_pred) * target).sum()
        tpr = tp / n_pos
        tnr =  tn / n_neg
        f1 =  2*tp / (2*tp + fn + fp)

        log_dict = {
            'termination_pred/tpr': tpr,
            'termination_pred/tnr': tnr,
            'termination_pred/f1': f1,
        }
        return loss, log_dict
    ...
    def setup_trainer(self, cfg=None):
        ...
        self.scheduler = torch.optim.lr_scheduler.LinearLR(self.optimizer, start_factor=1., end_factor=1., total_iters=20000)
