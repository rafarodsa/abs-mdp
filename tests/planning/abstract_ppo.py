'''
    Test PPO with learned abstract MDP 
...
def grounding_goal_fn(grounding, goal=[0.55, 0.06, 0., 0.], goal_tol=0.01):
    goal = torch.Tensor(goal).unsqueeze(0)
    def _goal(state):
        in_ = torch.cat([torch.from_numpy(state[:2]), torch.zeros(4)], dim=0).unsqueeze(0)
        log_prob = grounding.distribution(in_).log_prob(goal)
        return log_prob.exp() > 0.5
    return _goal
...
def main():
    ...
    parser.add_argument(
        '--absmdp',
        type=str,
        help='Path to the abstract MDP file'
    )
    ...
    model_features = nn.Sequential(
        lecun_init(nn.Linear(env.latent_dim, 128)),
        nn.ReLU(),
        lecun_init(nn.Linear(128, 128)),
        nn.ReLU()
    )

    model = OptionPolicyInit(
                        model_features,
    ...
    agent = PPO(
        ...
        opt,
        gpu=args.gpu,
        phi=lambda x: torch.from_numpy(x).float(),
    ...
    if args.load:
        ...
        agent.model = AbstractPolicyWrapper(agent.model, env.encoder)
